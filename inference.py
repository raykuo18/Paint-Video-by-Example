...
def main():
    ...
    parser.add_argument(
        "--plms",
        action='store_true',
        help="use plms sampling (alternate method that promises good diffusion results in fewer steps)",
    )
    parser.add_argument(
        "--fixed_code",
        action='store_true',
    ...
    if opt.raw_image != "":
        # Detect face and save face and mask
        face_detect(opt.raw_image)
    ...
    with torch.no_grad():
        with precision_scope("cuda"):
            with model.ema_scope():
                ...
                # print('inpaint_mask', [z_inpaint.shape[-2],z_inpaint.shape[-1]])
                ...
                if not opt.skip_save:
                    for i,x_sample in enumerate(x_checked_image_torch):
                        ...
                        img.save(f"{opt.raw_image}_grid.png")
